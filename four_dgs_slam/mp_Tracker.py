...
class Tracker(SLAMParameters):
    def __init__(self, slam, output_queue=None):
        super().__init__()
        ...
        self.input_queue = slam.input_queue
        ...
        # Camera poses
        if slam.trajmanager is not None:
             self.trajmanager = slam.trajmanager
             self.poses = [self.trajmanager.gt_poses[0]]
        else:
             self.trajmanager = None
             self.poses = [np.eye(4)]
    ...
    def tracking(self):
        ...
        if self.input_queue is None:
            ...
            self.rgb_images = []
            self.depth_images = []
            self.num_images = 99999999 # Infinite
        ...
        pbar = tqdm(total=self.num_images) if self.input_queue is None else None
        ...
        while True:
            ...
            if self.input_queue is not None:
                try:
                    ...
                    if data is None:
                        break
                    # Supporting (rgb, depth) or (rgb, depth, timestamp) or (rgb, depth, timestamp, pose)
                    if len(data) == 4:
                        current_image, depth_image, current_timestamp, external_pose = data
                    elif len(data) == 3:
                        current_image, depth_image, current_timestamp = data
                    else:
                        current_image, depth_image = data
                        current_timestamp = time.time()
                except Exception as e:
                    print(f"Error getting data from queue: {e}")
                    break
            else:
                if self.iteration_images >= self.num_images:
                    break
                current_image = self.rgb_images.pop(0)
                depth_image = self.depth_images.pop(0)
            
            self.iter_shared[0] = self.iteration_images
            ...
            # GICP or External Pose
            if self.iteration_images == 0:
                if external_pose is not None:
                     current_pose = external_pose
                     self.poses[-1] = current_pose # Update initial pose
                else:
                ...
                T = current_pose_inv[:3,3]
                R = current_pose_inv[:3,:3].transpose()
                ...
                # Push initial pose to output queue
                if self.output_queue is not None:
                    self.output_queue.put((self.poses[-1], current_timestamp))
                ...
                # Push pose to output queue
                if self.output_queue is not None:
                     self.output_queue.put((current_pose, current_timestamp))
                ...
                if distances.shape[0] > 0:
                     ratio = len_corres/distances.shape[0]
                else:
                     ratio = 0

                if  (self.input_queue is None and self.iteration_images >= self.num_images-1) \
                    or ratio < self.keyframe_th:
        ...
        if self.input_queue is None and self.num_images > 0:
    ...
    def get_images(self, images_folder):
        rgb_images = []
        depth_images = []
        if self.trajmanager and self.trajmanager.which_dataset == "replica":
        ...
        elif self.trajmanager and self.trajmanager.which_dataset == "tum":
