...
class GS_ICP_SLAM(SLAMParameters):
    def __init__(self, args, input_queue=None, output_queue=None):
        super().__init__()
        ...
        # Load camera parameters from file if config is a string (path), otherwise assume it's set later or via args
        if isinstance(self.config, str) and os.path.exists(self.config):
            ...
            self.camera_parameters = [str(self.W), str(self.H), str(self.fx), str(self.fy), str(self.cx), str(self.cy), str(self.depth_scale), str(self.depth_trunc), "dummy"]
        ...
        # TrajManager is used for GT comparison. Disable if no dataset path or not replica/tum
        if self.dataset_path and os.path.exists(self.dataset_path):
             try:
                 ...
                 num_final_poses = 2000 # Dummy
        else:
            self.trajmanager = None
            num_final_poses = 10000 # Large buffer for live SLAM

        # Make test cam - needed to init shared memory?
        # We need to initialize shared objects with SOME size.
        # If no test image, use dummy
        if self.dataset_path and os.path.exists(os.path.join(self.dataset_path, "images")):
             test_rgb_img, test_depth_img = self.get_test_image(f"{self.dataset_path}/images")
        else:
             test_rgb_img = np.zeros((self.H, self.W, 3), dtype=np.uint8)
             test_depth_img = np.zeros((self.H, self.W), dtype=np.float32)

        test_points, _, _, _ = self.downsample_and_make_pointcloud(test_depth_img, test_rgb_img)
        
        # Get size of final poses
        # num_final_poses = len(self.trajmanager.gt_poses) # Moved above
        ...
        self.shared_new_points = SharedPoints(test_points.shape[0] if test_points.shape[0] > 0 else 1000)
        self.shared_new_gaussians = SharedGaussians(test_points.shape[0] if test_points.shape[0] > 0 else 1000)
        ...
        self.tracker = Tracker(self, output_queue=self.output_queue)
    ...
    def downsample_and_make_pointcloud(self, depth_img, rgb_img):
        ...
        # Handle empty/init points
        if self.downsample_idxs[0].shape[0] == 0:
